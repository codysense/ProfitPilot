...
type CreatePurchaseReturnFormData = z.infer<typeof createPurchaseReturnSchema>;
...
const CreatePurchaseReturnModal = ({
  ...
  } = useForm<CreatePurchaseReturnFormData>({
    resolver: zodResolver(createPurchaseReturnSchema),
    defaultValues: { purchaseId, returnLines: [] },
  });
  ...
  const { data: returnable, isLoading } = useQuery({
    queryKey: ["returnable-purchase-lines", purchaseId],
    queryFn: () => purchaseApi.getReturnableLines(purchaseId),
  });
  ...
  const onSubmit = async (data: CreatePurchaseReturnFormData) => {
    ...
    try {
      await purchaseApi.createPurchaseReturn({
        ...data,
          ...
          <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg leading-6 font-medium text-gray-900">
                Create Purchase Return — {returnable?.purchase?.orderNo}
              </h3>
              <button
                onClick={onClose}
