...
type UpdateTransferFormData = z.infer<typeof updateTransferSchema>;
...
const EditTransferModal = ({
  ...
  const {
    ...
    defaultValues: {
      transferItems: transfer.items.map((item: any) => ({
        itemId: item.itemId,
        qty: Number(item.qty),
      })),
    },
  ...
  const { data: stockData } = useQuery({
    queryKey: ["edit-transfer-stock", transfer.fromWarehouseId, watchedItems],
    queryFn: async () => {
      if (!transfer.fromWarehouseId || !watchedItems.length) return {};
    ...
    enabled: !!(
      transfer.fromWarehouseId && watchedItems.some((item) => item.itemId)
    ),
  });

  const [isReceiving, setIsReceiving] = React.useState(false);

  const handleUpdate = async (data: UpdateTransferFormData) => {
    try {
      await inventoryApi.updateInventoryTransfer(transfer.refId, {
  ...
  const handleReceive = async () => {
    try {
      ...
      await inventoryApi.updateInventoryTransfer(transfer.refId, {
        transferItems: currentValues.transferItems,
      });

      // Then receive
      await inventoryApi.receiveInventoryTransfer(transfer.refId);
      toast.success("Inventory transfer received successfully");
      onSuccess();
    } catch (error) {
          ...
          <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg leading-6 font-medium text-gray-900">
                Edit & Receive Transfer: {transfer.refId}
              </h3>
              <button
                onClick={onClose}
            ...
            <div className="bg-blue-50 p-4 rounded-md mb-6">
              <div className="flex items-center">
                ...
                <div className="text-sm text-blue-800">
                  <strong>Transfer Direction:</strong> {transfer.fromWarehouse?.name} → {transfer.toWarehouse?.name}
                </div>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit(handleUpdate)} className="space-y-6">
                          ...
                          <div className="mt-1 p-2 bg-white border border-gray-200 rounded-md text-sm">
                            {(() => {
                              ...
                              const isInsufficient =
                                stock < (watchedItems[index]?.qty || 0);

                              return (
                                <div
                                  className={`flex items-center ${isInsufficient ? "text-red-600" : "text-green-600"}`}
                                >
                                  <span>
              ...
              <div className="flex justify-between items-center pt-4 border-t">
                ...
                <div className="flex space-x-3">
                  <button
                    type="submit"
                    disabled={isSubmitting || isReceiving}
                    className="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none disabled:opacity-50"
                  >
                  ...
                  >
                    ...
                    {isReceiving ? "Receiving..." : "Receive Transfer"}
                  </button>
                </div>
              </div>
            </form>
