import { PrismaClient, CostingMethod, LedgerDirection } from '@prisma/client';
import { Decimal } from '@prisma/client/runtime/library';

const prisma = new PrismaClient();

export class CostingService {
  async getCostingMethod(itemId: string): Promise<CostingMethod> {
  ...
  private async issueFifo(
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const totalAvailable = batches.reduce((sum, batch) => sum.plus(batch.qtyOnHand), new Decimal(0));
  ...
  async getInventoryValue(itemId: string, warehouseId: string): Promise<{ qty: number; value: number; avgCost: number }> {
    const costingMethod = await this.getCostingMethod(itemId);
