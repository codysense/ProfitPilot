...
const production_1 = require("../types/production");
...
class ProductionController {
    ...
    async createProductionOrder(req, res) {
        try {
            const validatedData = production_1.createProductionOrderSchema.parse(req.body);
    ...
    async releaseProductionOrder(req, res) {
        try {
            const { id } = req.params;
    ...
    async issueMaterials(req, res) {
        try {
            const { id } = req.params;
            const validatedData = production_1.issueMaterialsSchema.parse(req.body);
            await prisma.$transaction(async (tx) => {
                let totalIssueValue = 0;
                // Issue each material
                for (const material of validatedData.materials) {
                    const result = await costingService.issueInventory(material.itemId, (await tx.productionOrder.findUnique({ where: { id }, select: { warehouseId: true } })).warehouseId, material.qty, 'PRODUCTION', id, req.user.id);
    ...
    async addLabor(req, res) {
        try {
            const { id } = req.params;
            const validatedData = production_1.addLaborSchema.parse(req.body);
            await prisma.$transaction(async (tx) => {
    ...
    async addOverhead(req, res) {
        try {
            const { id } = req.params;
            const validatedData = production_1.addOverheadSchema.parse(req.body);
            await prisma.$transaction(async (tx) => {
                // Record in WIP ledger
                await tx.wipLedger.create({
                    data: {
    ...
    async receiveFinishedGoods(req, res) {
        try {
            const { id } = req.params;
            const validatedData = production_1.receiveFinishedGoodsSchema.parse(req.body);
            await prisma.$transaction(async (tx) => {
                const order = await tx.productionOrder.findUnique({
                    where: { id },
                    include: { item: true }
                ...
                if (validatedData.qtyGood > 0) {
                    await costingService.receiveInventory(order.itemId, order.warehouseId, validatedData.qtyGood, unitCost, 'PRODUCTION', id, req.user.id);
                }
    ...
    async finishProductionOrder(req, res) {
        try {
            const { id } = req.params;
            const order = await prisma.productionOrder.update({
                where: { id },
    ...
    async deleteProductionOrder(req, res) {
        try {
            const { id } = req.params;
    ...
    async printProductionOrder(req, res) {
        try {
            const { id } = req.params;
            const order = await prisma.productionOrder.findUnique({
                where: { id },
