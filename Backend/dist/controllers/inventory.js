...
class InventoryController {
    ...
    async createBom(req, res) {
        try {
            const validatedData = inventory_1.createBomSchema.parse(req.body);
    ...
    async adjustInventory(req, res) {
        try {
            const validatedData = inventory_1.inventoryAdjustmentSchema.parse(req.body);
            await prisma.$transaction(async (tx) => {
                if (validatedData.adjustmentType === 'IN') {
                    // Inventory increase
                    await costingService.receiveInventory(validatedData.itemId, validatedData.warehouseId, validatedData.qty, validatedData.unitCost || 0, 'ADJUSTMENT', `ADJ-${Date.now()}`, req.user.id);
                    ...
                    // Inventory decrease
                    const result = await costingService.issueInventory(validatedData.itemId, validatedData.warehouseId, validatedData.qty, 'ADJUSTMENT', `ADJ-${Date.now()}`, req.user.id);
                    // Post to general ledger
                    await glService.postJournal([
    ...
    async transferInventory(req, res) {
        try {
            const validatedData = inventory_1.inventoryTransferSchema.parse(req.body);
    ...
    async getInventoryLedger(req, res) {
        try {
            const { page = 1, limit = 20, itemId, warehouseId, userId, itemType, refType, direction, dateFrom, dateTo } = req.query;
            const skip = (Number(page) - 1) * Number(limit);
            const where = {};
            if (itemId)
                where.itemId = itemId;
            ...
            if (dateFrom || dateTo) {
                ...
                if (dateTo)
                    where.postedAt.lte = new Date(dateTo);
            }
            if (itemType) {
    ...
    async exportInventoryLedger(req, res) {
        try {
            const { format = 'csv', itemId, warehouseId, userId, itemType, refType, direction, dateFrom, dateTo } = req.query;
            const where = {};
            if (itemId)
                where.itemId = itemId;
            if (warehouseId)
    ...
    async getWarehousesList(req, res) {
        try {
    ...
    async createWarehouse(req, res) {
        try {
            const validatedData = inventory_1.createWarehouseSchema.parse(req.body);
    ...
    async createLocation(req, res) {
        try {
            const validatedData = inventory_1.createLocationSchema.parse(req.body);
