...
class CashController {
    ...
    // Update cash account
    async updateCashAccount(req, res) {
        try {
            const { id } = req.params;
    ...
    // Delete cash account
    async deleteCashAccount(req, res) {
        try {
            const { id } = req.params;
    ...
    async getCashTransactions(req, res) {
        try {
            const { page = 1, limit = 20, cashAccountId, transactionType, dateFrom, dateTo } = req.query;
    ...
    async createCashTransaction(req, res) {
        try {
            const { cashAccountId, glAccountId, contraAccountId, transactionType, amount, description, transactionDate, reference } = req.body;
                        ...
                        data: [
                            {
                                ...
                                debit: new library_1.Decimal(amount),
                            ...
                            },
                            {
                                ...
                                credit: new library_1.Decimal(amount),
                                refType: 'CASH_TRANSACTION',
                                refId: cashTransaction.id
                            }
                        ]
    ...
    async createCustomerPayment(req, res) {
        try {
            const { customerId, cashAccountId, amount, paymentDate, reference, notes, saleId } = req.body;
            const result = await prisma.$transaction(async (tx) => {
                ...
                const receipt = await tx.salesReceipt.create({
                    data: {
                        ...
                        customerId,
                        cashAccountId,
                        amountReceived: new library_1.Decimal(amount),
    ...
    async getCashbook(req, res) {
        try {
            const { page = 1, limit = 20, cashAccountId, dateFrom, dateTo } = req.query;
            const skip = (Number(page) - 1) * Number(limit);
            const where = {};
            if (cashAccountId)
                where.cashAccountId = cashAccountId;
