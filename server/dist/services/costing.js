...
class CostingService {
    ...
    async getCostingMethod(tx, itemId) {
    ...
    async issueWeightedAverage(tx, itemId, warehouseId, qty, refType, refId, userId) {
        // Get current running totals
        const lastEntry = await tx.inventoryLedger.findFirst({
            where: { itemId, warehouseId },
            orderBy: { postedAt: 'desc' }
    ...
    async issueFifo(tx, itemId, warehouseId, qty, refType, refId, userId) {
    ...
    async getInventoryValue(itemId, warehouseId) {
        const costingMethod = await prisma.item.findUnique({
            where: { id: itemId },
            select: { costingMethod: true }
        }).then(item => item?.costingMethod ?? client_1.CostingMethod.WEIGHTED_AVG);
        if (costingMethod === client_1.CostingMethod.WEIGHTED_AVG) {
            ...
            const totalValue = batches.reduce((sum, b) => sum + b.qtyOnHand.toNumber() * b.unitCost.toNumber(), 0);
