...
class ManagementService {
    ...
    async updateCompanySettings(data, userId) {
    ...
    async updateSystemSetting(data, userId) {
    ...
    async updateCostingPolicy(method, userId) {
    ...
    async activateFiscalPeriod(periodId) {
        return await prisma.$transaction(async (tx) => {
    ...
    async closeFiscalPeriod(periodId) {
        ...
        return await prisma.$transaction(async (tx) => {
            const workflow = await tx.approvalWorkflow.create({
                data: {
                    name: data.name,
                    entity: data.entity,
                    minAmount: data.minAmount ? new library_1.Decimal(data.minAmount) : null,
                    maxAmount: data.maxAmount ? new library_1.Decimal(data.maxAmount) : null
                }
            });
    ...
    async getApprovalRequests(userId, status) {
        const where = {};
        if (userId)
    ...
    async getEntityForApproval(entityType, entityId) {
        switch (entityType) {
            case 'PURCHASE_ORDER':
    ...
    async findApplicableWorkflow(entityType, amount) {
    ...
    async updateEntityApprovalStatus(tx, entityType, entityId, status) {
        const updateData = {
    ...
    async getUsersWithDetails(page = 1, limit = 10, search) {
    ...
    async updateChartAccount(accountId, data) {
    ...
    async deleteChartAccount(accountId) {
    ...
    async generateAccountCode(accountType, tx) {
        ...
        const usedCodes = new Set(existingCodes.map((acc) => parseInt(acc.code)));
