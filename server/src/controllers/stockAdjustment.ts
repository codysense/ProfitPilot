...
export class AdjustmentController {
 ...
 async getStockAdjustment  (req: AuthRequest, res: Response) {
  try {
    ...
    return res.status(500).json({ success: false, error: "Failed to fetch stock adjustments" });
  }
};
...
async adjustStock(req: AuthRequest, res: Response) {
    try {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        const totalQuantity = validatedData.adjustmentLines.reduce((sum, line) => {
    ...
    if (adjustmentLine) {
      ...
      const accountCode = await getAccountCodeById(validatedData.accountId)
      ...
      if (adjustmentLine.adjustmentType ==='SURPLUS'){
          ...
          await glService.postJournal([
            { accountCode:   itemType ==='FINISHED_GOODS'? '1350':'1300', debit: Number(cogs), credit: 0, refType: 'STOCK ADJUSTMENT', refId: id },
            { accountCode: String(accountCode), debit: 0, credit: Number(cogs), refType: 'STOCK ADJUSTMENT', refId: id },
            
          ], `Stock adjustment: ${newAdjustment.id}`, req.user!.id);
        }else{
          ...
          await costingService.issueInventory(
            ...
            await glService.postJournal([
                { accountCode:  itemType ==='FINISHED_GOODS'? '1350': '1300', debit: 0, credit: Number(cogs), refType: 'STOCK ADJUSTMENT', refId: id },
                { accountCode: String(accountCode), debit: Number(cogs), credit:0 , refType: 'STOCK ADJUSTMENT', refId: id },
